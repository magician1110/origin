...
<xsl:stylesheet 
	...
	<xsl:param name="modifyPath"/>
	<xsl:variable name="modify" select="replace($modifyPath, '\\', '/')"/>
	<xsl:variable name="content" select="collection(concat('file:///', $modify, '/XML?select=*.xml'))"/>
	<xsl:variable name="callout" select="collection(concat('file:///', $modify, '/%5BCallout%5D/?select=*.xml'))"/>
