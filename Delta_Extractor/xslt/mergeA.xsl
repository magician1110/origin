...
<xsl:stylesheet 
	...
	<xsl:param name="beforePath"/>
	<xsl:variable name="before" select="replace($beforePath, '\\', '/')"/>
	<xsl:variable name="content" select="collection(concat('file:///', $before, '/?select=*.xml'))"/>
	<xsl:variable name="callout" select="collection(concat('file:///', $before, '/%5BCallout%5D/?select=*.xml'))"/>
	...
	<xsl:template match="stories">
		<xsl:text>&#xA;&#x9;&#x9;</xsl:text>
		<stories filename="{ast:getFilename(document-uri(/), '/')}">
			<xsl:apply-templates select="@* | node()"/>
			<xsl:text>&#xA;&#x9;&#x9;</xsl:text>
		</stories>
	</xsl:template>
