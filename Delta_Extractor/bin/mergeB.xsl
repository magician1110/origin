...
<xsl:stylesheet 
	...
	<xsl:param name="afterPath"/>
	<xsl:variable name="after" select="replace($afterPath, '\\', '/')"/>
	<xsl:variable name="content" select="collection(concat('file:///', $after, '/?select=*.xml'))"/>
	<xsl:variable name="callout" select="collection(concat('file:///', $after, '/%5BCallout%5D/?select=*.xml'))"/>
