...
public class Extractor implements Runnable {
	...
	private int start;
	private int finish;
	ArrayList<String> highlightTextArr = new ArrayList<String>();
	...
	private void processHighlight(PDAnnotationTextMarkup highlight, PDPage page) throws IOException {
		...
		System.out.println(highlightText.replaceAll("\r\n|\t",""));
		highlightTextArr.add(highlightText.replaceAll("\r\n|\t",""));
		highlight.setContents(highlightText.replaceAll("\r\n|\t",""));

	}
	
	public void fileWrite(){//C:\workspace\MMI_AutoMatchingNew		
		File fileWiter = new File("C:\\test999");	
		BufferedWriter bw = null;		
		try {
			bw = new BufferedWriter(new FileWriter(fileWiter));
			//writer = new FileWriter(file, false);
			if(null != bw){				
				for(int j = 0; j < highlightTextArr.size(); j++){
					bw.write(highlightTextArr.get(j));
					bw.write("\n");
				}
				bw.flush();
				bw.close();
