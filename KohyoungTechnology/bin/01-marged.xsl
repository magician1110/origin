...
<xsl:stylesheet 
	...
	<xsl:variable name="target" select="replace($targetPath, '\\', '/')"/>

	<!-- <xsl:variable name="dummyPath" select="collection(concat(string-join(tokenize(document-uri(/), '/')[position() ne last()], '/'), '/../resource/?select=index.html;recurse=yes'))"/> -->
	<xsl:variable name="dummyPath" select="collection(concat('file:///', $target, '/?select=index.html;recurse=yes'))"/>
	
	<!-- <xsl:variable name="infoPath" select="collection(concat(string-join(tokenize(document-uri(/), '/')[position() ne last()], '/'), '/../resource/?select=Info.html;recurse=yes'))"/> -->
	...
	<xsl:variable name="infoTitle">
		<xsl:for-each select="$eachInfoPath/path/@src">
			...
			<xsl:variable name="splitPath">
				...
				<!-- <xsl:variable name="rootPath" select="replace(functs:getpath(base-uri(document('')), '/'), 'xsls', 'resource/')"/> -->
				<!-- <xsl:variable name="rootPath" select="$target"/> -->
